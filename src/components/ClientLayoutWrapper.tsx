                ...
                <header className="theme-nav sticky top-0 z-50">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className="flex h-10 w-10 items-center justify-center border border-neutral-300 bg-transparent text-[10px] font-bold tracking-[0.18em] text-black dark:border-white/10 dark:text-white">LOGO</div>
                            <span className="text-[20px] font-medium tracking-tight text-black dark:text-white">Thesis Module</span>
                        </div>
    ...
    return (
        <SidebarProvider defaultOpen={true}>
            <div className="flex h-screen w-full overflow-hidden bg-white dark:bg-[#040404]">
                ...
                <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-gray-200 dark:border-white/10 bg-white dark:bg-[#050505]">
                    <div className="flex h-16 items-center border-b border-gray-200 dark:border-white/10 px-4">
                        <div className="flex items-center gap-3">
                            <div className="flex h-8 w-8 items-center justify-center border border-neutral-300 bg-transparent text-[8px] font-bold tracking-[0.18em] text-black dark:border-white/10 dark:text-white">Study</div>
                            <span className="text-lg font-medium tracking-tight text-black dark:text-white">Study Hub</span>
                        </div>
                    </div>
                ...
                {/* Mobile Menu Button */}
                <button onClick={() => setMobileMenuOpen(true)} className="fixed left-4 top-4 z-50 flex h-10 w-10 items-center justify-center rounded-md border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#050505] md:hidden">
                        ...
                        <div className="fixed left-0 top-0 z-50 h-full w-64 transform animate-in slide-in-from-left duration-300 md:hidden">
                            <div className="flex h-full flex-col bg-white dark:bg-[#050505] shadow-xl">
                                <div className="flex h-16 items-center justify-between border-b border-gray-200 dark:border-white/10 px-4">
                                    ...
                                    <button onClick={() => setMobileMenuOpen(false)} className="rounded-md p-1 hover:bg-gray-100 dark:hover:bg-white/10">
                ...
                <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                    <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-gray-200 bg-white dark:border-white/10 dark:bg-[#050505]" style={{ paddingLeft: "1rem", paddingRight: "2rem" }}>
                        {/* Breadcrumbs */}
                        <div className="flex items-center gap-2">
                            <nav className="flex items-center gap-1 text-sm">
                                {breadcrumbs.map((crumb, index) => (
                                    <div key={crumb.href} className="flex items-center gap-1">
                                        {index === 0 && <Home className="h-3.5 w-3.5 text-gray-500 dark:text-gray-400" />}
                                        {crumb.isLast ? (
                                            <span className="font-medium text-gray-900 dark:text-white">{crumb.label}</span>
                                        ) : (
                                            <>
                                                <Link href={crumb.href} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">
                                                    {crumb.label}
                                                </Link>
                            ...
                            {authUser && (
                                <>
                                    ...
                                    <button className="flex h-8 items-center justify-center rounded-full border border-gray-200 bg-gray-50 dark:border-white/10 dark:bg-white/5 px-4">
                                        <span className="text-sm font-medium text-black dark:text-white">{authUser?.user?.profile?.name ?? authUser?.user?.username ?? "User"}</span>
                                    </button>
                                    <a href="/api/auth/logout" title="Sign out" className="flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 bg-gray-50 text-gray-600 transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-600 dark:border-white/10 dark:bg-white/5 dark:text-gray-400 dark:hover:border-red-500/30 dark:hover:bg-red-500/10 dark:hover:text-red-400">
